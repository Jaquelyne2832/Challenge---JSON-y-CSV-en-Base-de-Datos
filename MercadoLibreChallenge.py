    ...
    try:
        ...
        if filesize == 0:
            if not os.path.exists(CSV_PATH):
                print("No se encontro el CSV con nombre: user_manager.csv en la ruta: " + str(APP_PATH))
                return 0
            
            if not os.path.exists(JSON_PATH):
                print("No se encontro el json con nombre: dblist.json en la ruta: " + str(APP_PATH))
                return 0
